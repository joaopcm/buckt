...
export class HttpClient {
  ...
  private async handleResponse<T>(res: Response): Promise<ApiResponse<T>> {
    ...
    if (!res.ok) {
      const message = (body as ApiErrorResponse).error?.message ?? res.statusText
      switch (res.status) {
        ...
        default: throw new BucktError(message, res.status)
      }
    }
  ...
  async put<T>(path: string, body: BodyInit, contentType?: string): Promise<ApiResponse<T>> {
    const res = await fetch(new URL(path, this.baseUrl).toString(), {
      method: "PUT",
