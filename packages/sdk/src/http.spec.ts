...
import { NotFoundError, UnauthorizedError, ForbiddenError, ValidationError, ConflictError } from "./errors"
...
describe("HttpClient", () => {
  ...
  it("sends Authorization header", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({ Authorization: "Bearer bkt_test123" }),
      }),
    )
  })
  ...
  it("throws ValidationError on 400", async () => {
    vi.stubGlobal("fetch", mockFetch(400, { data: null, error: { message: "Bad input" }, meta: null }))
    const client = new HttpClient("https://api.test.com", "bkt_test")
    await expect(client.get("/test")).rejects.toThrow(ValidationError)
  })

  it("throws UnauthorizedError on 401", async () => {
    vi.stubGlobal("fetch", mockFetch(401, { data: null, error: { message: "Invalid key" }, meta: null }))
    const client = new HttpClient("https://api.test.com", "bkt_test")
    await expect(client.get("/test")).rejects.toThrow(UnauthorizedError)
  })

  it("throws ForbiddenError on 403", async () => {
    vi.stubGlobal("fetch", mockFetch(403, { data: null, error: { message: "No access" }, meta: null }))
    const client = new HttpClient("https://api.test.com", "bkt_test")
    await expect(client.get("/test")).rejects.toThrow(ForbiddenError)
  })

  it("throws NotFoundError on 404", async () => {
    vi.stubGlobal("fetch", mockFetch(404, { data: null, error: { message: "Not found" }, meta: null }))
    const client = new HttpClient("https://api.test.com", "bkt_test")
    await expect(client.get("/test")).rejects.toThrow(NotFoundError)
  })

  it("throws ConflictError on 409", async () => {
    vi.stubGlobal("fetch", mockFetch(409, { data: null, error: { message: "Conflict" }, meta: null }))
    const client = new HttpClient("https://api.test.com", "bkt_test")
    await expect(client.get("/test")).rejects.toThrow(ConflictError)
  })

  it("sends JSON body for POST", async () => {
    const fetch = mockFetch(201, { data: { id: "1" }, error: null, meta: null })
    vi.stubGlobal("fetch", fetch)
